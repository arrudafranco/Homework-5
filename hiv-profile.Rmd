...
```{r reports}
reports <- tibble(
  country = unique(hiv$iso3),
  filename = stringr::str_c("hiv-profile-", country, ".html"),
  params = purrr::map(country, ~ list(my_iso3 = .))
)

reports %>% 
  select(output_file = filename, params) %>% 
  purrr::pwalk(rmarkdown::render, input = "hiv-profile.Rmd")
```
...
plot2_title <- glue::glue("Year: {most_recent}")
hiv %>%
